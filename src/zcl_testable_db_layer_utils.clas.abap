class ZCL_TESTABLE_DB_LAYER_UTILS definition
  public
  final
  create public .
...
CLASS ZCL_TESTABLE_DB_LAYER_UTILS IMPLEMENTATION.
  ...
  METHOD check_ends_with.
    ...
    lv_ends_with = zcl_testable_db_layer_utils=>to_upper_case( iv_ends_with ).
  ...
  method CHECK_STARTS_WITH.
    ...
    lv_starts_with = zcl_testable_db_layer_utils=>to_upper_case( iv_starts_with ).
  ...
  method CLEAR_QUOTES_FROM_VALUE.
    ...
    rv_value = iv_value.
    IF rv_value(1) = ''''
      AND zcl_testable_db_layer_utils=>get_last_n_chars( iv_string              = rv_value
  ...
  method DELETE_START_WORD.
    SPLIT iv_sql_source AT space INTO zcl_testable_db_layer_utils=>dummy rv_sql_without_word.
  endmethod.
  ...
  method GET_START_WORD.
    SPLIT iv_sql AT space INTO rv_first_word zcl_testable_db_layer_utils=>dummy.
  endmethod.
  ...
  METHOD raise_exception_from_sy_msg.
    ...
    RAISE EXCEPTION TYPE zcx_testable_db_layer
      EXPORTING
        textid = ls_t100_key.
  ENDMETHOD.
  ...
  method RAISE_IF_TRANSP_TAB_NOT_EXIST.
    ...
    IF sy-subrc <> 0.
      MESSAGE e062 WITH iv_table_name INTO zcl_testable_db_layer_utils=>dummy.
      zcl_testable_db_layer_utils=>raise_exception_from_sy_msg( ).
    ENDIF.

    IF lv_tabclass <> 'TRANSP'.
      MESSAGE e063 WITH iv_table_name INTO zcl_testable_db_layer_utils=>dummy.
      zcl_testable_db_layer_utils=>raise_exception_from_sy_msg( ).
    ENDIF.
  endmethod.
