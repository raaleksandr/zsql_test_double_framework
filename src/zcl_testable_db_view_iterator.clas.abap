class ZCL_TESTABLE_DB_VIEW_ITERATOR definition
  public
  inheriting from ZCL_TESTABLE_DB_SELECT_ITER
  create public .
...
CLASS ZCL_TESTABLE_DB_VIEW_ITERATOR IMPLEMENTATION.
  ...
  method CONSTRUCTOR.
    ...
    IF is_database_view( iv_view_name ) <> abap_true.
      MESSAGE e067 WITH iv_view_name INTO zcl_testable_db_layer_utils=>dummy.
      zcl_testable_db_layer_utils=>raise_exception_from_sy_msg( ).
    ENDIF.

    lv_view_name = zcl_testable_db_layer_utils=>to_upper_case( iv_view_name ).
  ...
  method IS_DATABASE_VIEW.
    ...
    lv_viewname = zcl_testable_db_layer_utils=>to_upper_case( iv_name_of_object ).
  ...
  METHOD _get_from_sql_part.
    ...
    IF zcl_testable_db_layer_utils=>check_ends_with( iv_sql       = rv_from_sql_part
                                                     iv_ends_with = 'AND' ) = abap_true.

      rv_from_sql_part = zcl_testable_db_layer_utils=>delete_end_word_if_equals( iv_sql_source         = rv_from_sql_part
