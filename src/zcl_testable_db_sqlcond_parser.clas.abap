class ZCL_TESTABLE_DB_SQLCOND_PARSER definition
  public
  inheriting from ZCL_TESTABLE_DB_PARSER_BASE
  abstract
  create public .

public section.

  interfaces ZIF_TESTABLE_DB_SQLCOND_PARSER .
...
protected section.
  ...
  types:
    BEGIN OF TY_block,
           parser TYPE REF TO zif_testable_db_sqlcond_parser,
         END OF ty_block .
  types:
...
private section.
  ...
  types:
    BEGIN OF ty_one_condition,
           ...
           ref_to_parser    TYPE REF TO zcl_testable_db_where_parser,
...
CLASS ZCL_TESTABLE_DB_SQLCOND_PARSER IMPLEMENTATION.


  method ZIF_TESTABLE_DB_SQLCOND_PARSER~CHECK_CONDITION_FOR_CUR_REC.
    ...
    IF ms_not_condition-parser IS BOUND.
      rv_condition_true =
        zcl_testable_db_layer_utils=>boolean_not(
  ...
  METHOD zif_testable_db_sqlcond_parser~parse_condition.
    ...
    IF zcl_testable_db_layer_utils=>check_starts_with( iv_sql         = iv_sql_condition
                                                       iv_starts_with = c_not ) = abap_true.

      DATA: lv_condition_without_not TYPE string.

      ms_not_condition-parser = zif_testable_db_sqlcond_parser~get_parser_instance( ).
      lv_condition_without_not =
        zcl_testable_db_layer_utils=>delete_start_word_if_equals( iv_sql_source           = iv_sql_condition
    ...
    IF _has_brackets_around( iv_sql_condition ) = abap_true.
      ...
      zif_testable_db_sqlcond_parser~parse_condition( iv_sql_condition = lv_condition_no_brackets ).
      RETURN.
    ENDIF.
  ...
  METHOD _check_elementary.
    ...
    CASE m_operation.
      ...
      WHEN 'NE' OR '<>'.
        ...
        ENDIF.
      WHEN c_in.
        IF zcl_testable_db_layer_utils=>is_internal_table( <lv_value_or_right_operand> ) = abap_true.
      ...
      WHEN 'LIKE'.
        ...
        ENDIF.
      WHEN OTHERS.
        MESSAGE e066 WITH m_operation INTO zcl_testable_db_layer_utils=>dummy.
  ...
  method _CLEAR_QUOTES_FROM_VALUE.
    m_fieldname_right_or_value = zcl_testable_db_layer_utils=>clear_quotes_from_value( m_fieldname_right_or_value ).
  endmethod.
  ...
  METHOD _delete_brackets_around.
    ...
    IF _has_brackets_around( cv_sql_condition ) = abap_true.
      cv_sql_condition = zcl_testable_db_layer_utils=>condense( cv_sql_condition ).
  ...
  METHOD _get_ref_to_condition_operand.
    ...
    DATA: ls_data_set            TYPE zcl_testable_db_sqltab_iterpos=>ty_data_set,
  ...
  method _HAS_BRACKETS_AROUND.
    ...
    lv_sql_condition = zcl_testable_db_layer_utils=>condense( iv_sql_condition ).
    ...
    IF lv_sql_condition(1) = '(' AND zcl_testable_db_layer_utils=>get_last_n_chars( iv_string              = lv_sql_condition
  ...
  METHOD _parse_elementary.
    ...
    lv_condition = zcl_testable_db_layer_utils=>to_upper_case( iv_sql_condition ).
  ...
  method _SPLIT_BY_AND_FILL_CONDITIONS.
    ...
    IF ev_condition_found = abap_true.
      LOOP AT lt_parts_after_split INTO lv_part.
        ...
        <ls_condition>-parser = zif_testable_db_sqlcond_parser~get_parser_instance( ).
  ...
  method _SPLIT_BY_OR_AND.
    ...
    LOOP AT ltd_words INTO lv_word.
      lv_word_upper = zcl_testable_db_layer_utils=>to_upper_case( lv_word ).
