class ZCL_TESTABLE_DB_SELECT_PARSER definition
  public
  create public .
...
public section.
  ...
  methods PARSE_FIELD_LIST_IN_SELECT
    importing
      ...
      !IO_SQLTABLES_ITERATOR type ref to ZCL_TESTABLE_DB_SQLTABLES_ITER
...
CLASS ZCL_TESTABLE_DB_SELECT_PARSER IMPLEMENTATION.
  ...
  method GET_RESULT_MOVE_CORRESPONDING.
    ...
    ASSIGN md_data_set->* TO <lt_result>.
    zcl_testable_db_layer_utils=>move_corresponding_table( EXPORTING it_table_src  = <lt_result>
  ...
  method _APPEND_TO_LAST_FIELD.
    ...
    lv_alias = zcl_testable_db_layer_utils=>condense( iv_alias ).
    ...
    IF zcl_testable_db_layer_utils=>check_starts_with( iv_sql         = lv_alias
  ...
  METHOD _fill_dataset_where_empty.
    ...
    DATA: lt_data_set_list          TYPE zcl_testable_db_sqltab_iterpos=>ty_data_sets,
    ...
    LOOP AT mt_select_parameters ASSIGNING <ls_select_parameter>
      ...
      IF <ls_select_parameter>-dataset_name IS INITIAL AND <ls_select_parameter>-function_name <> c_function_count.
        MESSAGE e057 WITH <ls_select_parameter>-field_name INTO zcl_testable_db_layer_utils=>dummy.
    ...
    LOOP AT <lt_table_before> ASSIGNING <ls_line_before>.
      ...
      IF sy-subrc = 0.
        LOOP AT mt_select_parameters ASSIGNING <ls_select_parameter>.
          ...
          ENDIF.
          lv_fieldname = zcl_testable_db_layer_utils=>to_upper_case( lv_fieldname ).
          ...
          ELSEIF <ls_select_parameter>-parameter_type = parameter_type-field.
            ...
            IF sy-subrc <> 0.
              MESSAGE e061 WITH lv_fieldname INTO zcl_testable_db_layer_utils=>dummy.
              zcl_testable_db_layer_utils=>raise_exception_from_sy_msg( ).
            ENDIF.
          ENDIF.
        ENDLOOP.
  ...
  METHOD _parse_field.
    ...
    cs_parameter-dataset_name = zcl_testable_db_layer_utils=>condense( zcl_testable_db_layer_utils=>to_upper_case( lv_dataset_name_or_alias ) ).
    cs_parameter-field_name   = zcl_testable_db_layer_utils=>condense( zcl_testable_db_layer_utils=>to_upper_case( lv_field_name ) ).
  ENDMETHOD.
  ...
  method _PARSE_FUNCTION.
    ...
    LOOP AT ltd_parts INTO lv_part.
      IF _is_function_name( lv_part ) = abap_true.
        cs_parameter-function_name = zcl_testable_db_layer_utils=>to_upper_case( lv_part ).
  ...
  method _RUN_GROUP_BY_FUNCTION.
    ...
    CASE iv_group_by_function.
      ...
      WHEN OTHERS.
        MESSAGE e060 WITH iv_group_by_function INTO zcl_testable_db_layer_utils=>dummy.
        zcl_testable_db_layer_utils=>raise_exception_from_sy_msg( ).
    ENDCASE.
  endmethod.
  ...
  method _SEPARATE_ALIAS.
    ...
    LOOP AT lt_words INTO lv_word.
      ...
      IF zcl_testable_db_layer_utils=>to_upper_case( lv_word ) = 'AS'.
  ...
  METHOD _split_select_into_fields.
    ...
    ELSE.
      ...
      LOOP AT lt_fields_in_result_set_copy INTO lv_field_after_split.
        ...
        IF zcl_testable_db_layer_utils=>to_upper_case( lv_field_after_split ) = c_as.
        ...
        IF lv_add_field = abap_true.
          IF lv_field IS NOT INITIAL.
            APPEND zcl_testable_db_layer_utils=>condense( lv_field ) TO rt_fields_in_result_set.
          ENDIF.
  ...
  method _STAR_TO_FIELD_LIST.
    ...
    DATA: lt_new_select_parameters LIKE mt_select_parameters,
          lt_data_set_list         TYPE zcl_testable_db_sqltab_iterpos=>ty_data_sets,
          ls_data_set              LIKE LINE OF lt_data_set_list,
          lt_components            TYPE cl_abap_structdescr=>component_table,
