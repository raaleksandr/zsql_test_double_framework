*&---------------------------------------------------------------------*
*& Report ZTESTABLE_DB_EXAMPLE_DBCHANGE
*&---------------------------------------------------------------------*
*& Example program updating database
*& Demonstrates unit test of database updates with
*& ZTESTABLE_DB_LAYER lib
*&---------------------------------------------------------------------*

REPORT ztestable_db_example_dbchange MESSAGE-ID ztestable_db_layer.
  ...
  PUBLIC SECTION.
    METHODS:
      ...
      update_carrier IMPORTING is_changed_scarr TYPE scarr
                     RAISING   zcx_testable_db_layer,
      mass_delete_of_carriers IMPORTING it_ids_of_carriers_to_delete TYPE ztestable_db_layer_carr_id_tab
                              RAISING   zcx_testable_db_layer.
...
CLASS lcl_controller_and_view DEFINITION.
  ...
  PRIVATE SECTION.
    ...
    METHODS: _alv_on_user_command FOR EVENT added_function OF cl_salv_events
      ...
      _display_error IMPORTING io_exception TYPE REF TO zcx_testable_db_layer,
      _refresh_alv RAISING zcx_testable_db_layer.
ENDCLASS.
...
CLASS lcl_model IMPLEMENTATION.
  ...
  METHOD insert_new_carrier.
    ...
    IF sy-subrc = 0.
      MESSAGE e701 WITH is_new_scarr-carrid INTO zcl_testable_db_layer_utils=>dummy.
  ...
  METHOD update_carrier.
    ...
    IF sy-subrc <> 0.
      MESSAGE e705 WITH is_changed_scarr-carrid INTO zcl_testable_db_layer_utils=>dummy.
      zcl_testable_db_layer_utils=>raise_exception_from_sy_msg( ).
    ENDIF.
  ...
  METHOD mass_delete_of_carriers.
    ...
    LOOP AT it_ids_of_carriers_to_delete ASSIGNING <ls_id_of_carrier>.
      ...
      IF sy-subrc <> 0.
        MESSAGE e708 WITH <ls_id_of_carrier>-carrid INTO zcl_testable_db_layer_utils=>dummy.
        zcl_testable_db_layer_utils=>raise_exception_from_sy_msg( ).
      ENDIF.
    ENDLOOP.
    ...
    IF lv_subrc = 0.
      MESSAGE e706 INTO zcl_testable_db_layer_utils=>dummy.
      zcl_testable_db_layer_utils=>raise_exception_from_sy_msg( ).
    ENDIF.
    ...
    IF lv_subrc = 0.
      MESSAGE e707 INTO zcl_testable_db_layer_utils=>dummy.
      zcl_testable_db_layer_utils=>raise_exception_from_sy_msg( ).
    ENDIF.
  ...
  METHOD _check_record.
    ...
    IF is_scarr_to_check-carrname IS INITIAL.
      MESSAGE e702 WITH 'Name of Carrier' INTO zcl_testable_db_layer_utils=>dummy.
      zcl_testable_db_layer_utils=>raise_exception_from_sy_msg( ).
    ENDIF.
    ...
    IF is_scarr_to_check-currcode IS NOT INITIAL.
      ...
      IF lv_subrc <> 0.
        MESSAGE e703 WITH is_scarr_to_check-currcode INTO zcl_testable_db_layer_utils=>dummy.
        zcl_testable_db_layer_utils=>raise_exception_from_sy_msg( ).
      ENDIF.
    ENDIF.
    ...
    IF is_scarr_to_check-url NP 'HTTP*'
      ...
      MESSAGE e704 WITH is_scarr_to_check-url INTO zcl_testable_db_layer_utils=>dummy.
      zcl_testable_db_layer_utils=>raise_exception_from_sy_msg( ).
    ENDIF.
  ENDMETHOD.
ENDCLASS.
...
CLASS lcl_controller_and_view IMPLEMENTATION.
  ...
  METHOD _insert_new_record.
    ...
    DATA: ls_new_record TYPE scarr,
          lo_exception  TYPE REF TO zcx_testable_db_layer.
    ...
    TRY.
      ...
      CATCH zcx_testable_db_layer INTO lo_exception.
  ...
  METHOD _delete_records.
    DATA: lo_selections          TYPE REF TO cl_salv_selections,
          ...
          lt_ids_to_delete       TYPE TABLE OF ztestable_db_layer_carr_id,
...
*?</asx:abap>
  PRIVATE SECTION.
    ...
    METHODS: setup,
      insert_successful FOR TESTING RAISING zcx_testable_db_layer,
      "
      insert_error_already_exist FOR TESTING RAISING zcx_testable_db_layer,
      insert_error_empty_carrname FOR TESTING RAISING zcx_testable_db_layer,
      insert_error_wrong_currency FOR TESTING RAISING zcx_testable_db_layer,
      insert_error_wrong_url FOR TESTING RAISING zcx_testable_db_layer,
      "
      update_successful FOR TESTING RAISING zcx_testable_db_layer,
      "
      update_error_carr_not_found FOR TESTING RAISING zcx_testable_db_layer,
      update_error_empty_carrname FOR TESTING RAISING zcx_testable_db_layer,
      "
      delete_successful FOR TESTING RAISING zcx_testable_db_layer,
      "
      delete_error_carr_not_exists FOR TESTING RAISING zcx_testable_db_layer,
      delete_error_carrid_in_sbook FOR TESTING RAISING zcx_testable_db_layer,
      "
      delete_multi_successful FOR TESTING RAISING zcx_testable_db_layer.
ENDCLASS.
...
CLASS ltc_unit_tests_on_prog IMPLEMENTATION.
  ...
  METHOD delete_successful.
    ...
    " WHEN
    DATA: lt_scarr_ids_to_delete TYPE ztestable_db_layer_carr_id_tab.
